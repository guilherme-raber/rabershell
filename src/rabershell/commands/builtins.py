...
from rabershell.shell.models import CommandResult, CommandRuntime, ResultAction


def help_command(runtime: CommandRuntime, args: tuple[str, ...]) -> CommandResult:
    if len(args) > 1:
        return CommandResult('Uso: ajuda [comando]', is_error=True)
    return runtime.render_help(args[0] if args else None)


def clear_command(runtime: CommandRuntime, args: tuple[str, ...]) -> CommandResult:
    del runtime
    if args:
...
def version_command(runtime: CommandRuntime, args: tuple[str, ...]) -> CommandResult:
    del runtime
    if args:
...
def exit_command(runtime: CommandRuntime, args: tuple[str, ...]) -> CommandResult:
    del runtime
    if args:
...
def enter_icmp_command(runtime: CommandRuntime, args: tuple[str, ...]) -> CommandResult:
    if args:
...
def back_command(runtime: CommandRuntime, args: tuple[str, ...]) -> CommandResult:
    if args:
